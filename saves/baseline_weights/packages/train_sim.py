...
from packages.val_sim import val_sim
...
from models.extractor import JavascriptSimilarity
...
def train_sim(args):
    ...
    if args.load is None:
        model = JavascriptSimilarity(args, vocab)
    else:
    ...
    for epoch in range(args.epochs):
        ...
        for i, (inputs, lengths, labels, oovs) in enumerate(data_loader):
            ...
            sim = model(sources, queries, lengths, targets)  # [batch x seq x vocab]
            ...
            string = "[%d]: Epoch %d\t%d/%d\tSimilarity - loss: %1.3f, acc: %1.3f" \
                         % (steps, epoch + 1, i, total_batches, loss.data[0], acc2)
            print(string)
            ...
            if steps % 10 == 0:
                val_sim(model,vocab,args,steps)
            if steps % 500 ==0:
